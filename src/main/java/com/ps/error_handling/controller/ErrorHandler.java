 ...
 * This class handles exceptions thrown by the application and converts them into appropriate HTTP responses.
 * It uses the {@link RestControllerAdvice} annotation to intercept exceptions thrown by controllers
 * and provides centralized exception handling across all controllers.
 */
...
public class ErrorHandler {
    ...
    private static final URI REPORT_CLIENT_ERROR_TYPE = URI.create("https://api.error-handling.com/problems/report-client-error");
    private static final URI INVALID_REPORT_NAME_TYPE = URI.create("https://api.error-handling.com/problems/invalid-report-name");
    private static final URI USER_NOT_FOUND_TYPE = URI.create("https://api.error-handling.com/problems/user-not-found");
    private static final URI REPORT_NOT_FOUND_TYPE = URI.create("https://api.error-handling.com/problems/report-not-found");

    /**
     * Handles exceptions related to database access errors.
     * This method is called when a {@link DatabaseAccessException} is thrown.
    ...
    /**
     * Handles exceptions related to unexpected errors from the report client.
     * This method is called when a {@link ReportClientUnexpectedException} is thrown.
     * It returns a 500 Internal Server Error response with a generic error message.
     *
     * @param ex the exception that was thrown
     * @return a {@link ProblemDetail} with status code 500 and a generic error message
    ...
    /**
     * Handles exceptions related to invalid report names.
     * This method is called when a {@link ReportNameNotFoundInReportApiException} is thrown.
     * It returns a 400 Bad Request response with an error message indicating the report name is invalid.
     *
     * @param ex the exception that was thrown
    ...
    /**
     * Handles exceptions related to users not found in the database.
     * This method is called when a {@link UserNotFoundInDbException} is thrown.
     * It returns a 404 Not Found response with an error message indicating the user ID doesn't exist.
     *
     * @param ex the exception that was thrown
    ...
    /**
     * Handles exceptions related to users not found in the report API.
     * This method is called when a {@link UserNotFoundInReportApiException} is thrown.
     * It returns a 404 Not Found response with an error message indicating the report for the user was not found.
     *
     * @param ex the exception that was thrown
